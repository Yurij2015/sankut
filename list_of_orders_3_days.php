...
<section id="admin" class="container text-center">
    ...
    <div class="container">
        <div class="center">
            ...
            <p class="lead">Список заказов за последние 3 дня</p>
        </div>

        <?php

        class Show_orders_3d
        {
            public $id;
            public $name;
            public $email;
        ...
        $sql = "SELECT id, name, email, phone_number, service.name_of_service, furniture.name_of_furniture, wishes FROM orders, furniture, service WHERE service.id_service = orders.service and furniture.id_furniture = orders.furniture and  date > NOW() - INTERVAL 3 DAY";
        $reception = $pdo->query($sql);
        $rows = $reception->fetchAll(PDO::FETCH_CLASS, "Show_orders_3d");
